...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub = nodeHandler.subscribe("/zed/zed_node/rgb/image_rect_color", 1, &ImageGrabber::GrabImage,&igb);
...
void ImageGrabber::GrabImage(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    cv::Mat imu;
    cv::Mat Tcw = mpSLAM->TrackMonocular(cv_ptr->image,cv_ptr->header.stamp.toSec()); // Tcw is the current pose (position + orientation)
    int state = mpSLAM->GetTrackingState(); // OK = 2
    vector<ORB_SLAM2::MapPoint*> vMPs = mpSLAM->GetTrackedMapPoints();
    vector<cv::KeyPoint> vKeys = mpSLAM->GetTrackedKeyPointsUn();

    cv::undistort(im,imu,K,DistCoef); //un distortion 去形變

    if(bRGB)
        viewerAR.SetImagePose(imu,Tcw,state,vKeys,vMPs);
    else
        cout << "camera is not RGB type" << endl;
